 ...
 * - Turret: auto-aims toward hub center using Limelight
 * - Flywheel: locked at 6000 RPM, D-pad Up decreases by 300 RPM per press
 * - Pivot: operator adjusts angle via D-pad Left/Right
 * - Shoot: right trigger to fire (auto when RPM >= 5600)
 *
...
public class ShooterTuningCommand extends Command {
    ...
    public ShooterTuningCommand(XboxController controller,
        ...
        m_flywheel = flywheel;
        m_feeder = feeder;
        addRequirements(pivot, flywheel, feeder);
    }
    ...
    public void execute() {
        ...
        if (pov != m_lastPOV) {
            switch (pov) {
                ...
                case POV_DOWN: m_currentPivotAngle = Math.max(0, m_currentPivotAngle - PIVOT_STEP); break;
            }
        }
        m_lastPOV = pov;
